...
*/
  ...
  for (key in metrics.counters) {
    ...
    if (listKeys[0] != 'statsd') {
      array_counts.push({
      "source": listKeys[0] || '',
      "event":listKeys[1] || '',
      "application":listKeys[2] || '',
      "rule":listKeys[3] || '',
      "object_type":listKeys[4] || '',
      "object":listKeys[5] || '',
      "adaptor":listKeys[6] || '',
      "action_type":listKeys[7] || '',
      "value":value,
      "timestamp": ts
      });
      numStats += 1;
    } else if (debug.toString() == 'true'){
      console.log("Did not write counter stats:", value);
    };
  };
  ...
  for (key in metrics.timer_data) {
    ...
    value["event"] = listKeys[1] || '';
    value["application"] = listKeys[2] || '';
    value["rule"] = listKeys[3] || '';
    value["object_type"] = listKeys[4] || '';
    value["object"] = listKeys[5] || '';
    value["adaptor"] = listKeys[6] || '';
    value["action_type"] = listKeys[7] || '';
    if (value['histogram']) {
